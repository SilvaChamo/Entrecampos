...
import { User, Mail, Lock, ChevronRight, Loader2 } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Erro ao registar conta. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#f0f0f1] flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white shadow-xl border border-[#ccd0d4] overflow-hidden">
        <div className="bg-[#1d2327] p-8 text-white text-center">
          ...
          <p className="text-gray-400 text-sm">Crie a sua conta para aceder ao dashboard</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                className="w-full px-4 py-2 border border-gray-300 focus:ring-2 focus:ring-[#2271b1] focus:border-transparent outline-none transition-all bg-white"
                value={formData.role}
                onChange={(e) => setFormData({ ...formData, role: e.target.value as UserRole })}
              >
                <option value="guest">Visitante (Visualização e Planos)</option>
                <option value="contribuidor">Contribuidor (Escrever Notícias e Partilhar Ficheiros)</option>
              </select>
            </div>
          </div>
          ...
          <p className="text-center text-sm text-gray-500">
            Já tem uma conta? <Link href="/login" className="text-[#2271b1] hover:underline font-medium">Faça login</Link>
          </p>
        </form>
      </div>
    </div>
