...
const HeaderOld: React.FC = () => {
  ...
  const [offCanvasMode, setOffCanvasMode] = useState<'menu' | 'login'>('menu');
          ...
          <div className="flex-1 overflow-y-auto">
            {offCanvasMode === 'login' ? (
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <h3 className="text-sm font-semibold text-gray-800 mb-3">Criar Conta</h3>
                  <form className="space-y-2">
                    <input
                      type="text"
                    ...
                    <button
                      type="submit"
                      className="w-full bg-red-600 text-white py-2 text-sm font-medium rounded-lg hover:bg-red-700 transition-colors"
                    >
                      Registar
                    </button>
                  </form>
              ...
              /* Modo Menu - Menu Normal + Contacto + Rodapé */
              <nav className="p-4 space-y-2">
                    ...
                    {item.hasDropdown ? (
                      <div className="space-y-0">
                        <button
                          ...
                          className={`flex items-center justify-between w-full px-4 py-[8px] text-left text-sm font-medium rounded-lg transition-colors ${
                            isActive(item.href)
                              ? 'text-red-600'
                        ...
                        {dropdownOpen === item.name && item.items && (
                          <div className="ml-4 space-y-0">
                            {item.items.map((subItem) => (
                              <Link
                                key={subItem.name}
                                href={subItem.href}
                                className={`block px-4 py-[8px] text-sm rounded-lg transition-colors ${
                      ...
                      <Link
                        href={item.href}
                        className={`block px-4 py-[8px] text-sm font-medium rounded-lg transition-colors ${
                          isActive(item.href)
                            ? 'text-red-600'
                            : 'text-gray-700 hover:text-red-600'
                        }`}
                ...
                <div className="mt-4 border-t border-gray-100 pt-4">
                  <form className="space-y-2 bg-gray-50 border border-gray-200 rounded-lg p-3">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all resize-none"
                    />
                    <button
                      type="submit"
                      className="w-full bg-green-600 text-white py-2 text-sm font-medium rounded-lg hover:bg-green-700 transition-colors"
                ...
                {/* Rodapé com Subscrever e Redes Sociais */}
                <div className="mt-4 p-4 -mx-4 mb-0" style={{ backgroundColor: '#2d2d2d' }}>
                  <div className="flex items-center justify-between gap-3">
                    {/* Botão Subscrever à esquerda */}
                    <Link
                      href="#"
                      className="bg-red-600 text-white px-5 py-3 rounded-lg text-sm font-medium hover:bg-red-700 transition-colors"
                      onClick={() => setOffCanvasOpen(false)}
                    >
                      Subscrever
                    </Link>
