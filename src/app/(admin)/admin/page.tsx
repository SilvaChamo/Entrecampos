...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        pending: 3, // Mock
        reviewed: 12, // Mock
        contributions: 5 // Mock
      });

      // 3. Fetch Recent Activities (Latest News)
  ...
  return (
    <div className="bg-[#f0f0f1] min-h-screen text-[#2c3338] font-sans pb-12">
      {/* 1. Welcome Section - Largura Total */}
      <section className="bg-white border border-[#ccd0d4] p-6 mb-5 shadow-sm rounded-none">
        <div className="max-w-[1200px] mx-auto">
          <h1 className="text-[23px] font-normal text-[#1d2327] mb-2">
            {role === 'guest' ? 'Bem-vindo ao EntreCAMPOS' : `Bem-vindo ao Painel de Administração, ${user?.name}`}
          </h1>
          <p className="text-[14px] text-[#50575e] mb-4">
            {role === 'guest' ? 'Junte-se a nós para partilhar as suas notícias e multimedia.' : 'Reunimos alguns links para começar:'}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            <div>
              ...
              <Link 
                ...
                className="inline-block px-6 py-2.5 bg-[#2271b1] text-white rounded-[10px] text-[14px] font-bold hover:bg-[#135e96] transition-all shadow-sm mb-4"
              >
