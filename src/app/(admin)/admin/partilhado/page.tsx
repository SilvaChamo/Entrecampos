...
import { 
  ...
  Folder, ChevronDown, ChevronLeft
} from 'lucide-react';
...
export default function PartilhadoPage() {
  ...
  const sharedItems: SharedItem[] = [
    ...
    { id: 2, name: 'Colheita Milho.jpg', type: 'image', size: '2.3 MB', contributor: 'Maria Santos', contributorEmail: 'maria@email.com', date: '2026-04-24', description: 'Fotos da colheita', isGuest: false },
    ...
    { id: 5, name: 'Dica Irrigação.jpg', type: 'image', size: '1.5 MB', contributor: 'Visitante (Email)', contributorEmail: 'anonimo@email.com', date: '2026-04-21', description: 'Sistema caseiro partilhado', isGuest: true },
    ...
    { id: 7, name: 'Plantio Café.jpg', type: 'image', size: '3.1 MB', contributor: 'João Silva', contributorEmail: 'joao@email.com', date: '2026-04-19', description: 'Processo de plantio', isGuest: false },
    ...
    { id: 9, name: 'Guia Práticas.docx', type: 'document', size: '850 KB', contributor: 'Pedro Costa', contributorEmail: 'pedro@email.com', date: '2026-04-17', description: 'Guia completo', isGuest: false },
  ];
  ...
  const getContributorItems = () => {
    ...
    return sharedItems.filter(item => item.contributorEmail === selectedContributor.email);
  };
  ...
  return (
    <div className="p-6 text-[#2c3338]">
      {/* Header - Alinhado com botão voltar com fundo */}
      <div className="flex items-center gap-3 mb-6">
        {view !== 'filters' && (
          <button 
            onClick={handleBack} 
            className="flex items-center gap-2 px-3 py-2 bg-white border border-[#ccd0d4] rounded-lg hover:bg-[#f6f7f7] transition-colors"
          >
      ...
      {view === 'filters' && (
        <>
          {/* Filtro, Busca e Botão na mesma linha */}
          <div className="mb-6">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <div className="relative">
                <select
                  value={filterType}
                  onChange={(e) => setFilterType(e.target.value as 'all' | 'users' | 'emails')}
          ...
          {/* Galeria em 4 colunas - Priorizando imagens */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {[...sharedItems]
              .sort((a, b) => {
                // Priorizar imagens primeiro
              ...
              .map(item => (
                <div 
                  ...
                  style={{
                    backgroundImage: item.type === 'image' 
                      ? `linear-gradient(to bottom, rgba(0,0,0,0.1), rgba(0,0,0,0.6)), url(/api/placeholder/300/300)` 
                      : `linear-gradient(135deg, ${
                          item.type === 'video' ? '#dc2626' : '#2563eb'
                        }, ${
                          item.type === 'video' ? '#991b1b' : '#1d4ed8'
                        })`,
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                    ...
                    <p className="text-white/70 text-xs">{item.contributor}</p>
                  </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleContributorClick(contributor); }}
        ...
        <>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-[#1d2327]">{selectedContributor.email}</p>
                <p className="text-sm text-[#50575e]">{getContributorItems().length} ficheiros partilhados</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {getContributorItems().map((item, index) => (
              <div 
                key={item.id} 
                className="relative group h-[200px] rounded-lg overflow-hidden cursor-pointer"
                style={{
                  backgroundImage: item.type === 'image' 
                    ? `linear-gradient(to bottom, rgba(0,0,0,0.1), rgba(0,0,0,0.7)), url(/api/placeholder/400/300)` 
                    : `linear-gradient(135deg, ${
                        item.type === 'video' ? '#dc2626' : 
                        item.type === 'document' ? '#2563eb' : '#6b7280'
                      }, ${
                        item.type === 'video' ? '#991b1b' : 
                        item.type === 'document' ? '#1d4ed8' : '#374151'
                      })`,
                  backgroundSize: 'cover',
                  backgroundPosition: 'center'
                }}
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-between p-3">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-xs text-white/80">{item.size}</span>
                  </div>

                  {/* Baixo - Título à esquerda, botões à direita */}
                  <div className="flex items-end justify-between gap-2">
                    {/* Info - título e data à esquerda */}
                    <div className="flex-1 min-w-0">
                      <h4 className="text-white font-medium truncate text-sm" title={item.name}>
                        {item.name}
                      </h4>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button 
                        className="p-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded text-white transition-colors"
                        title="Visualizar"
                        onClick={(e) => {
                          e.stopPropagation();
                          alert(`Visualizar: ${item.name}`);
